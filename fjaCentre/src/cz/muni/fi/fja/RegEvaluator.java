...
public class RegEvaluator implements Evaluator {
  ...
  public boolean convertDevice(
      String p_source, String p_device, String p_destination) {
    prepareAttributes();
  ...
  public String getQuestion() {
    if (m_teacher.isError()) {
      return "Nemohu vygenerovat retezec pro odpovednik, nebyl zadan korektni model ucitele.";
    }
  ...
  private boolean equalDevices() {
    printTable(m_teacherTable, Messages.enterModel(), m_teacher);
    printTable(m_studentTable, Messages.enterModel(), m_student);
    ...
    if (!isError() || m_verbose) {
      ...
      if (m_verbose) {
        if (!m_teacher.isError()) {
          ...
          printTable(m_teacherTable, "Minimalizovany DFA", m_teacher);
        }
        if (!m_student.isError()) {
          m_student = m_student.makeMinimalCanonicDFA();
          printTable(m_studentTable, "Minimalizovany DFA", m_student);
        }
      }
  ...
  private boolean convertDevice() {
    ...
    if (NFA_EPSILON.equals(m_studentTask)) {
      if (DFA.equals(m_teacherTask)) {
        ...
        printTable(m_teacherTable, "DFA prevedeno na EFA.", m_teacher);
      } else {
      ...
      if (DFA.equals(m_teacherTask)) {
        m_teacher = new NFAFromDFA(m_teacher);
        printTable(m_teacherTable, "DFA prevedeno na NFA.", m_teacher);
      } else {
        m_teacher = convertDeviceToEFA(m_teacherTask, m_teacher, m_teacherTable);
        if (!GRAMMAR.equals(m_teacherTask)) {
          m_teacher = m_teacher.makeNFA();
          printTable(m_teacherTable, "Vysledny NFA bez epsilon kroku.", m_teacher);
        }
      }
    } else {
      if (DFA.equals(m_studentTask)) {
        ...
        printTable(m_teacherTable, "Vysledny DFA", m_teacher);
      } else {
        m_teacher = convertDeviceToDFA(m_teacherTask, m_teacher, m_teacherTable);
        if (DFA_MINIMAL.equals(m_studentTask)) {
          m_teacher = m_teacher.makeMinimalDFA();
          printTable(m_teacherTable, "Vysledny minimalizovany DFA", m_teacher);
          ...
          m_teacher = m_teacher.makeMinimalCanonicDFA();
          printTable(m_teacherTable, "Vysledny minimalizovany kanonicky DFA",
              m_teacher);
        } else if (DFA_CANONIC.equals(m_studentTask)) {
          m_teacher = m_teacher.makeCanonicDFA();
          printTable(m_teacherTable, "Vysledny kanonicky DFA", m_teacher);
        }
      }
    }
    return true;
  ...
  private RegularDevice convertDeviceToDFA(
    ...
    if (m_details) {
      printTable(p_sb, "NFA preveden na DFA", p_device);
    }
    return p_device;
  }
  ...
  private RegularDevice convertDeviceToNFA(
    ...
    if (!(GRAMMAR.equals(p_task) || NFA_EPSILON_FREE.equals(p_task))
      ...
      if (m_details) {
        printTable(p_sb, "NFA bez epsilon kroku", p_device);
      }
    }
    return p_device;
  }
  ...
  private RegularDevice convertDeviceToEFA(
    ...
    if (GRAMMAR.equals(p_task)) {
      p_device = p_device.makeNFA();
      printTable(p_sb, "Gramatika prevedena na NFA", p_device);
    } else if (REGULAR_EXPRESSION.equals(p_task)) {
      p_device = p_device.makeEFA();
      printTable(p_sb, "RE prevedeno na EFA.", p_device);
    }
    return p_device;
  }
  ...
  private void printTable(
      StringBuffer p_sb, String p_title, RegularDevice p_device) {
    if (m_verbose) {
      p_sb.append("<tr><td class=title>");
      p_sb.append(p_title);
      p_sb.append("</td></tr><tr><td><pre class=model>");
      assert p_device != null;
      p_sb.append(p_device.toString());
